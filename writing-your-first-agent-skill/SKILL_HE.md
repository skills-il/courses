...
- **`name`** חייב להיות ב־kebab-case, חייב להיות זהה לשם התיקייה בדיוק, וזה ה־slug שה־LLM משתמש בו פנימית כדי לזהות את הסקיל. בחרו שם מדויק: `id-validator`, לא `id-tools`. דרישה של המפרט.
...
זה הכל. הסקיל זמין עכשיו בסשנים שלכם ב־Claude Code. בלי פרסום, בלי ביקורת, בלי קטלוג. סקילים פרטיים תקפים לחלוטין ולעיתים קרובות מתאימים במיוחד לסקילים שקשורים לתהליך העבודה האישי שלכם.
...
2. **Push ל־repo פרטי ב־git.** הוא מבצע clone, ומתקין מה־clone המקומי עם אותה פקודה. קל יותר להישאר מסונכרנים תוך כדי שיפור (`git pull` + התקנה מחדש מושך את השינויים שלכם). עובד טוב לצוות.
