...
| 3. הרחבת הפורמט (אופציונלי) | מתי צריך יותר ממה שהמפרט נותן: מטא־דאטה לקטלוג, תוכן דו־לשוני, ודפוס ה־sidecar |
...
הפרק הזה מציג דפוס הרחבה אחד נפוץ: קובץ `metadata.json` בתור sidecar, עם קובץ ליווי לכל שפה. השדות הספציפיים למטה הם הקונבנציות של קטלוג ישראלי ציבורי אחד (agentskills.co.il) ושימושיים כדוגמת עבודה. קטלוגים אחרים בוחרים שדות אחרים. הלקח הוא דפוס ה־SIDECAR, לא הסכמה הספציפית. אם הסקיל שלכם פרטי לגמרי, אפשר לדלג על הפרק הזה לחלוטין.

### למה sidecar JSON ולא YAML מקונן

ה־YAML parser המחמיר של Claude Desktop דוחה מפתחות מקוננים בתוך frontmatter של SKILL.md לחלוטין, ומסרב לטעון את הסקיל בכלל. לכן הדפוס המקובל הוא להשאיר את ה־frontmatter של SKILL.md מינימלי (`name`, `description`, `license`, וזהו) ולמשוך את כל המטא־דאטה שספציפית לקטלוג לקובץ נפרד בשם `metadata.json` ליד SKILL.md. סביבות שקוראות רק את ה־frontmatter של SKILL.md (כמו Claude Desktop) טוענות נקי. כלי הקטלוג שקוראים את `metadata.json` מקבלים את המידע הדו־לשוני והקטלוגי שהם צריכים.
...
- **`display_name`** הוא הכותרת הדו־לשונית שמוצגת ב־UI של הקטלוג. עברית קודם כשמשלחים לקהל ישראלי.
- **`display_description`** הוא הקופי השיווקי הדו־לשוני. הגרסה לא־אנגלית חייבת להישמע כמו השפה הילידית, לא כמו תרגום (פרק 6 מרחיב).
...
- **`tags`** נשלחים כמערכים מקבילים לכל שפה. הקטלוגים בדרך כלל מסננים לפי התגיות באנגלית. המערך בשפה השנייה הוא לתצוגה ב־UI של אותה שפה.
...
כשאתם משלחים לקהל דו־לשוני, הקונבנציה המקובלת היא קובץ תרגום מקביל (`SKILL_HE.md` לעברית, מקובץ לפי קוד השפה שהקטלוג שלכם מצפה לו). שני הקבצים חולקים את אותה צורת YAML frontmatter (עם טקסט תיאור מתורגם), אותו מבנה גוף (כותרות פרקים מתורגמות), ואותן הפניות לתמונות.

הטבעיות של התרגום היא ההבדל האיכותי הכי גדול בין הסקילים הדו־לשוניים עם הכי הרבה התקנות לבין הזנב הארוך. הטעות הכי נפוצה כאן היא לכתוב SKILL.md, להעביר אותו בתרגום LLM, להדביק את הפלט לתוך הקובץ המתורגם, ולשלוח. התוצאה נקראת כמו אנגלית בתעתיק: סדר המילים שגוי, קלקות בכל מקום ("שכבת האסטרטגיה" ל־"the strategy layer", "שתי שאלות עוגן" ל־"two anchor questions"), ויש אי־הסכמות במין ("זו לא ייעוץ" צריך להיות "זה לא ייעוץ"). קוראים ילידיים מזהים את זה מיד.

הפתרון: אחרי הטיוטה הראשונה שה־LLM הפיק, שדובר ילידי (או pass עריכה ייעודי) יעבור על הטקסט לטבעיות. לעברית, תסתכלו על הסגנון של ערכי "כל זכות" בשפה פשוטה, על הסברי TheMarker, או על בלוגים ישראליים פופולריים בתחום שלכם.

### הפניה לסקילים אחרים בפרוזה לא־לטינית

כשהגוף המתורגם צריך להפנות לסקיל אחר (למשל כדי להמליץ על סקיל קשור), השתמשו בשם התצוגה המקומי ואחריו ה־slug בסוגריים, ולא ב־slug החשוף ב־kebab-case בתוך backticks. דוגמה בעברית:
...
הצורה הנכונה שומרת על זרימת הקריאה, ובמקביל משאירה את ה־slug במצב קריא למכונה לטובת כל כלי קישור. תוכן באנגלית משתמש בקונבנציית ה־slug החשוף ב־backticks. כתבים לא־לטיניים מרוויחים מהטיפול של שם תצוגה + slug.

### דוגמה ללימוד: סקיל דו־לשוני באיכות גבוהה

תמצאו סקיל דו־לשוני עם הרבה התקנות בקטלוג היעד שלכם והשוו בין `SKILL.md` שלו, קובץ הליווי לשפה השנייה, ו־`metadata.json`. סימן האיכות: התיאור לא־האנגלי נקרא טבעי, התגיות מקבילות כמו שצריך בין השפות, והגוף מכסה דאגות ספציפיות לתחום שאף חלופה גנרית לא הייתה מטפלת בהן.

הטעות הכי נפוצה בפרק 3: תרגום מילולי מאנגלית לשפה אחרת שנקרא שבור לקוראים ילידיים. הפתרון: לכתוב את הקובץ המתורגם כאילו הוא המקור, לא התרגום. חשיבה "שפת המקור קודם" מפיקה סקילים יותר טובים בצורה דרמטית.
...
3. **קלקות בעברית ב־`display_description` או בגוף המתורגם.** "שכבת האסטרטגיה", "שתי שאלות עוגן", סדר מילים מילולי מאנגלית. קורא ישראלי ילידי מזהה את זה ב־5 שניות.
...
4. תקראו את הגוף המתורגם בקול (או תנו לדובר ילידי לקרוא). זה נקרא טבעי, או כמו אנגלית מתורגמת?
