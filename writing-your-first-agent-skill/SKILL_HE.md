...
description: קורס בן 6 פרקים למפתחים על איך לכתוב agent skill מאפס לפי המפרט של Anthropic, שעובד ב־Claude Code, Cursor, Windsurf וב־Claude Desktop. מכסה את האנטומיה של SKILL.md, את ההבדל בין references/ ל־scripts/, איך לשתף את הסקיל, ואת 10 טעויות הכתיבה הנפוצות. מתאים לסקילים פרטיים, סקילים של צוות, ותרומות לקטלוגים ציבוריים.
license: MIT
---
...
הקורס הזה למפתחים שיודעים לכתוב קוד אבל עוד לא כתבו agent skill, או שכתבו אחד ורוצים להבין למה חלק מהסקילים נתפסים אמין על ידי Claude Code, Cursor, Windsurf או Claude Desktop, ואחרים יושבים בלי שימוש. שישה פרקים. דוגמאות קונקרטיות. הסקיל שתפיקו יכול להיות סקיל פרטי שאתם מחזיקים אצלכם, סקיל שאתם משתפים עם הצוות, או תרומה לקטלוג ציבורי (קטלוג skills-il הוא דוגמה שתראו לאורך הקורס). המכניקה זהה בכל שלושת המקרים.
...
agent skill של Claude הוא היחידה הקטנה ביותר של שינוי התנהגות LLM שאפשר לשלוח: קובץ Markdown אחד עם YAML frontmatter, אופציונלית מלווה בקבצי reference ובסקריפטים, שה־LLM קורא לפי דרישה כשהתיאור שלו תואם לבקשת המשתמש. זה כל המודל. כל השאר (קטלוג, sync pipeline, ציון אמון, מספרי התקנות) הוא תשתית סביב אותו קובץ בודד.
...
|---|---|---|
| **agent skill** | קובץ Markdown שה־LLM קורא לפי דרישה כדי ללמוד איך לטפל בסוג משימה ספציפי | אתם רוצים שה־LLM יידע איך לעשות משהו בלי שתחזרו על ההוראות בכל שיחה |
...
### המפרט של Anthropic Claude Skills, בפסקה אחת
...
השימוש הפשוט ביותר: להשאיר את הסקיל על המכונה שלכם. רוב הסביבות של Claude Skills נותנות להתקין סקיל מתיקייה מקומית. עם Claude Code:
